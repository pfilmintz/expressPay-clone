...
enum ActionDescriptor {
    case read, unread, more, flag, trash, star
            ...
            if style == .backgroundColor {
              //  let config = UIImage.SymbolConfiguration(pointSize: 20.0, weight: .regular)
               // return UIImage(systemName: name, withConfiguration: config)
                ...
                return circularIcon(with: color(forStyle: style), size: CGSize(width: 40, height: 40), icon: image)
            }
        } else {
    ...
    func color(forStyle style: ButtonStyle) -> UIColor {
        ...
        case .star: return UIColor.systemRed
        }
    #else
        switch self {
        ...
        case .star: return #colorLiteral(red: 1, green: 0.2352941176, blue: 0.1882352941, alpha: 1)
        }
    #endif
    }
